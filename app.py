...
def display_result_page(title, file_name, image_list):
    ...
    with st.expander("查看详细研究方法与流程说明", expanded=False):
        # 使用 st.text 解决您之前反馈的“内容不一致”问题（原样保留换行）
        st.text(description) 
...
if st.session_state.page == 'home':
    ...
    # --- 新增：从 introduction.txt 读取首页简介 ---
    st.header("研究背景与简介")
    intro_text = read_text_file("introduction.txt")
    # 首页简介通常需要格式化，所以保留 markdown
    # 如果首页也出现排版问题，可将其改为 st.text(intro_text)
    st.text(intro_text)
...
# --- 2. 其他分页面逻辑 ---
    ...
    if st.button("运行 MBDS 模型"):
        ...
        if "紊乱" in result: st.error(f"预测结果：{result}")
        else: st.success(f"预测结果：{result}")
